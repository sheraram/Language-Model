# -*- coding: utf-8 -*-
"""2019101034_MT-1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1bDDhV6WC5pBqEleCrx-FlyAl5hAAyeXn
"""
...
def text_to_ind(tokens):
  text_to_indices = []
  for token in tokens:
    x = english1.vocab.stoi[token]
    text_to_indices.append(x)
  return text_to_indices

def Transalate(model, sent, english1, french1, device, max_len):
  model.eval()

  tokens = tokenize(sent)
  text_to_indices = text_to_ind(tokens)
  ...
  for ind in range(0, max_len):
      # previous_word = torch.LongTensor([outputs[-1]]).to(device)
      previous_word = torch.LongTensor([outputs[-1]]).to(device).unsqueeze(0)
      ...
      # If 'end of the sentence' predicts by Model
  ...
  translated_sentence = []
  for idx in outputs:
    translated_sentence.append(french1.vocab.itos[idx])
  translated_sentence_output = translated_sentence[1:]

  return translated_sentence_output

sent="I was a Ph.D. student in clinical psychology"

" ".join(Transalate(model, sent, english1, french1, device, 50))
...
try:
  os.remove("2019101034_MT1_test.txt")
except:
  pass
...
try:
  os.remove("2019101034_MT1_train.txt")
except:
  pass

with open("2019101034_MT1_train.txt","a") as file:
...
with open("2019101034_MT1_test.txt","a") as file:
  file.write(str(corpus_bleu(french_actual, french_predict)))
  file.write("\n")
  for i in bleu_score:
        string=str(i[0])+"  "+str(i[1])
